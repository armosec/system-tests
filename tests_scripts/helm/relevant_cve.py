...
class RelevantCVEs(BaseVulnerabilityScanning):
    ...
    def start(self):
        ...
        self.verify_running_pods(namespace=namespace)
...
class RelevantCVEsBackendTest(BaseVulnerabilityScanning):
    ...
    def start(self):
        assert not self.backend, f'test "{self.test_driver.test_name}" must run with backend'
        ...
        self.test_cve_result(since_time=since_time, containers_scan_id=containers_scan_id,
                             ...
                             expected_number_of_pods=expected_number_of_pods)
...
class RelevantDataIsAppended(BaseVulnerabilityScanning):
    ...
    def start(self):
        ...
        Logger.logger.info('1. Test Image scanning completed')
        ...
        filteredCVEs, _ = self.wait_for_report(timeout=360, report_type=self.get_filtered_CVEs_from_storage,
                                               filteredCVEsKEys=self.get_filtered_data_keys(
                                                   ...
                                                   namespace=namespace))
        Logger.logger.info('Validate CVEs was created with expected data')
        self.validate_expected_filtered_CVEs(filteredCVEs, self.test_obj["expected_filtered_CVEs"],
                                             namespace=namespace)
...
class RelevancyEnabledDeletedImage(BaseVulnerabilityScanning):
    ...
    def start(self):
        ...
        workload_tags = self.get_workloads_images_tags(workload_objs, namespace=namespace)
        SBOMS = self.get_SBOM_from_storage(workload_tags)
...
class RelevancyEnabledExtraLargeImage(BaseVulnerabilityScanning):
    def __init__(self, test_obj=None, backend=None, kubernetes_obj=None, test_driver=None):
        super(RelevancyEnabledExtraLargeImage, self).__init__(test_driver=test_driver, test_obj=test_obj,
                                                              backend=backend,
                                                              kubernetes_obj=kubernetes_obj)
