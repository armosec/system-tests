...
class WorkflowsTeamsNotifications(Workflows):
    ...
    def start(self):
        ...
        workflow_body = self.build_vulnerabilities_workflow_body(name=VULNERABILITIES_WORKFLOW_NAME, severities=SEVERITIES_HIGH, channel_name=TEAMS_CHANNEL_NAME, channel_guid=get_env("TEAMS_CHANNEL_GUID"), cluster=self.cluster, namespace=None, category=VULNERABILITIES, cvss=6, webhook_url=get_env("WEBHOOK_URL"))
        self.create_and_assert_workflow(workflow_body, EXPECTED_CREATE_RESPONSE, update=False)
        workflow_body = self.build_compliance_workflow_body(name=COMPLIANCE_WORKFLOW_NAME, channel_name=TEAMS_CHANNEL_NAME, channel_guid=get_env("TEAMS_CHANNEL_GUID"), cluster=self.cluster, namespace=None, category=COMPLIANCE, driftPercentage=15, webhook_url=get_env("WEBHOOK_URL"))
        self.create_and_assert_workflow(workflow_body, EXPECTED_CREATE_RESPONSE, update=False)
        before_test_message_ts = time.time()

        Logger.logger.info("Stage 3: Validate workflows created successfully")
        self.validate_workflow(VULNERABILITIES_WORKFLOW_NAME, TEAMS_CHANNEL_NAME)
        self.validate_workflow(SECURITY_RISKS_WORKFLOW_NAME, TEAMS_CHANNEL_NAME)
        self.validate_workflow(COMPLIANCE_WORKFLOW_NAME, TEAMS_CHANNEL_NAME)
