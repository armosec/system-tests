...
class BaseKubescape(BaseK8S):
    ...
    def __init__(
        self, test_obj=None, backend=None, test_driver=None, kubernetes_obj=None
    ):
        super().__init__(
        ...
        self.remove_cluster_from_backend = self.test_driver.kwargs.get(
            "remove_cluster_from_backend", True
        )
    ...
    def create_kubescape_config_file(self):
        config_file_data = {statics.CLOUD_API_URL_KEY: "any_value"}
    ...
    def download_control_input(self):
        ...
        self.download_policy(
            output_file=output_file, policy="controls-inputs", account=True
        )
    ...
    def download_policy(
        self, output_file: str, policy: str, policy_name: str = None, **kwargs
    ):
        ...
        if "account" in kwargs:
            ...
            env = (
                {"KS_ACCESS_KEY": self.backend.get_access_key()}
                if self.backend.get_access_key() != ""
                else {}
            )
            command.extend(["--server", self.api_url])

        Logger.logger.info(" ".join(command))
        status_code, res = TestUtil.run_command(
        ...
        **kwargs,
        ...
        if "submit" in kwargs:
            ...
            account_id = (
                kwargs["account"]
                if "account" in kwargs
                else self.backend.get_customer_guid()
            )
            ...
            account_to_use = (
                tenant_id if tenant_id else self.backend.get_customer_guid()
            )
        ...
        if "use_artifacts" in kwargs and kwargs["use_artifacts"] != "":
            ...
            self.artifacts
        ):  # otherwise, load default artifacts (if passed by the command line)
        ...
        if "use_from" in kwargs and kwargs["use_from"] != "":
            ...
            self.policies
        ):  # otherwise, load default policies (if passed by the command line)
    ...
    def get_repository_posture_repositories(
        self, repository_owner: str, repository_name: str, repository_branch: str
    ):
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_repository_posture_repositories_by_name,
    ...
    def get_posture_frameworks(self, report_guid, framework_name: str = ""):
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_posture_frameworks,
    ...
    def get_framework(self, framework_name: str = ""):
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_framework, framework_name=framework_name
        )
        return c_panel_info

    def get_posture_controls(self, framework_name: str, report_guid):
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_posture_controls,
            framework_name=framework_name,
            report_guid=report_guid,
        )
        return c_panel_info
    ...
    def get_posture_resources(
        ...
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_posture_resources,
            framework_name=framework_name,
            report_guid=report_guid,
    ...
    def get_top_controls_results(self, cluster_name):
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_top_controls_results, cluster_name=cluster_name
        )
        return c_panel_info
    ...
    def get_posture_resources_by_control(
        self,
        related_exceptions: str,
        control_name: str,
        control_id: str,
        report_guid: str,
        framework_name: str,
        ...
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_posture_resources_by_control,
            framework_name=framework_name,
            report_guid=report_guid,
    ...
    def get_version_info(self, cluster_name: str):
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_cluster, cluster_name=cluster_name
        )
        assert isinstance(c_panel_info, dict), (
            f"Expecting cluster data to be a dict, but got the following: {json.dumps(c_panel_info)}"
        )
        return c_panel_info["apiServerInfo"]

    def get_customer_configuration(self):
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.get_customer_configuration
        )
        return c_panel_info

    def update_customer_configuration(self, customer_config: dict):
        c_panel_info, t = self.wait_for_report(
            report_type=self.backend.update_customer_configuration,
    ...
    def download_release(self, org: str = DEFAULT_ORG, release: str = DEFAULT_RELEASE):
        ...
        Logger.logger.debug(f"download url: {download_url}")
    ...
    def download_from_branch(self, org: str, branch: str):
        ...
        return_code, return_obj = TestUtil.run_command(
            command_args=["git", "clone", download_url, "-b", branch],
            cwd=self.test_driver.temp_dir,
            timeout=360,
        )
        assert not return_code, (
            f"Failed to clone kubescape from branch {branch} : {return_obj.stderr}"
        )
        ...
        return_code, return_obj = TestUtil.run_command(
            command_args=["go", "build", "-o", kubescape_exec], cwd=ks_path, timeout=360
        )
        assert not return_code, (
            f"Failed to build kubescape (go build) from branch {branch} : {return_obj.stderr}"
        )
    ...
    def test_zero_numbers_in_framework_result(framework_report: dict):
        ...
        assert framework_report[statics.FAILED_RESOURCES_COUNT_FIELD] != 0, (
            message.format(
                resource_name=framework_report["name"],
    ...
    def test_negative_numbers_in_result(framework_report: dict):
        ...
        """
        BaseKubescape.is_negative(
            result=framework_report[_CLI_SUMMARY_DETAILS_FIELD][
                _CLI_RESOURCE_COUNTERS_FIELD
            ],
            name="global",
        )
        for c_id, control in framework_report[_CLI_SUMMARY_DETAILS_FIELD][
            statics.CONTROLS_FIELD
        ].items():
            BaseKubescape.is_negative(
                result=control[_CLI_RESOURCE_COUNTERS_FIELD], name=c_id
            )

    @staticmethod
    def is_negative(result: dict, name: str):
        message = "found negative counter in: {resource_name}, counter: {counter}"
        assert result[_CLI_PASSED_RESOURCES_FIELD] >= 0, message.format(
            resource_name=name, counter=_CLI_PASSED_RESOURCES_FIELD
        )
        assert result[_CLI_FAILED_RESOURCES_FIELD] >= 0, message.format(
            resource_name=name, counter=_CLI_FAILED_RESOURCES_FIELD
        )
        # assert result[_CLI_EXCLUDED_RESOURCES_FIELD] >= 0, message.format(resource_name=name,
        #                                                                   counter=_CLI_EXCLUDED_RESOURCES_FIELD)

        assert result.get(_CLI_SKIPPED_RESOURCES_FIELD, 0) >= 0, message.format(
            resource_name=name, counter=_CLI_SKIPPED_RESOURCES_FIELD
        )

    @staticmethod
    def is_failed_passed_and_skipped_less_than_all(
        total_resources: int, resource_counters: dict, name: str
    ):
        message = (
            "in {name}, {all} < {passed}+{failed}+{skipped} (all<passed+failed+skipped)"
        )
    ...
    def test_resources_number_counter_in_result(framework_report: dict):
        ...
        BaseKubescape.is_failed_passed_and_skipped_less_than_all(
            ...
            resource_counters=framework_report[_CLI_SUMMARY_DETAILS_FIELD][
                _CLI_RESOURCE_COUNTERS_FIELD
            ],
            name="global",
        )
        for c_id, control in framework_report[_CLI_SUMMARY_DETAILS_FIELD][
            ...
            BaseKubescape.is_failed_passed_and_skipped_less_than_all(
                total_resources=all_resources,
                resource_counters=control[_CLI_RESOURCE_COUNTERS_FIELD],
                name=c_id,
            )
    ...
    def test_exception_result(framework_report: dict, controls_with_exception: list):
        for c_id in controls_with_exception:
            sub_status = framework_report[_CLI_SUMMARY_DETAILS_FIELD][
                statics.CONTROLS_FIELD
            ][c_id][_CLI_STATUS_INFO_FIELD][_CLI_SUB_STATUS_FIELD]
            assert sub_status == _CLI_SUB_STATUS_EXCEPTIONS, (
                "control {x} supposed to be {w}, but it is {y}".format(
                    x=c_id, y=sub_status, w=_CLI_SUB_STATUS_EXCEPTIONS
                )
            )
    ...
    def number_of_resources(framework_report: dict, resources: int):
        ...
        assert len(framework_report[_CLI_RESULTS_FIELD]) == resources, message.format(
            x=resources, y=len(framework_report[_CLI_RESULTS_FIELD])
        )
    ...
    def test_number_of_controls_in_be(cli_controls: dict, be_controls: list):
        ...
        assert len(be_controls) == len(cli_controls), message.format(
            x=len(cli_controls), y=len(be_controls)
        )
    ...
    def test_status_text(
        ...
        if failed_count > 0:
            ...
            assert status_txt == "failed", (
                "control {x} supposed to be failed, but it is {y}".format(
                    x=control_name, y=status_txt
                )
            )
        elif skipped_controls > 0:
            result_count["skipped_controls"] += 1
            assert status_txt == "skipped", (
                "control {x} supposed to be warning, but it is {y}".format(
                    x=control_name, y=status_txt
                )
            )
        else:
            ...
            assert (
                status_txt == "passed"
                or status_txt == "skipped"
                or status_txt == "warning"
                or status_txt == "irrelevant"
            ), "control {x} supposed to be passed or skipped, but it is {y}".format(
                x=control_name, y=status_txt
            )
    ...
    def test_controls_result_count(be_frameworks: dict, result_count: dict):
        assert (
            be_frameworks[statics.BE_TOTAL_CONTROLS_FILED]
            == result_count["total_controls"]
        ), (
            "Total-control from controls_api is {x}, but total-controls from framework_api is {y}.".format(
                x=result_count["total_controls"],
                y=be_frameworks[statics.BE_TOTAL_CONTROLS_FILED],
            )
        )
        assert (
            be_frameworks[statics.BE_FAILED_CONTROLS_FILED]
            == result_count["failed_controls"]
        ), (
            "Failed-control from controls_api is {x}, but failed-controls from framework_api is {y}.".format(
                x=result_count["failed_controls"],
                y=be_frameworks[statics.BE_FAILED_CONTROLS_FILED],
            )
        )
        assert (
            be_frameworks[statics.BE_WARNING_CONTROLS_FILED]
            == result_count["warning_controls"]
        ), (
            "Warning-control from controls_api is {x}, but warning-controls from framework_api is {y}.".format(
                x=result_count["warning_controls"],
                y=be_frameworks[statics.BE_WARNING_CONTROLS_FILED],
            )
        )

    @staticmethod
    def test_controls_from_backend(
        cli_controls: dict, be_frameworks: dict, be_controls: list
    ):
        BaseKubescape.test_number_of_controls_in_be(
            cli_controls=cli_controls, be_controls=be_controls
        )
        ...
        for be_control in be_controls:
            c_id = be_control["id"]
            assert c_id in cli_controls.keys(), (
                "control {x} found in be_results, but not in cli_results".format(x=c_id)
            )
            assert (
                be_control[statics.BE_FAILED_RESOURCES_COUNT_FIELD]
                == cli_controls[c_id][_CLI_RESOURCE_COUNTERS_FIELD][
                    _CLI_FAILED_RESOURCES_FIELD
                ]
            ), (
                "control {x} - {w}: the cli result is {y} failed, backend result is {z} failed".format(
                    x=c_id,
                    w=be_control[statics.BE_NAME_FILED],
                    y=cli_controls[c_id][_CLI_RESOURCE_COUNTERS_FIELD][
                        _CLI_FAILED_RESOURCES_FIELD
                    ],
            ...
            assert be_control.get(
                statics.BE_SKIPPED_RESOURCES_COUNT_FIELD, 0
            ) == cli_controls[c_id][_CLI_RESOURCE_COUNTERS_FIELD].get(
                _CLI_SKIPPED_RESOURCES_FIELD, 0
            ), (
                "control {x} - {w}: the cli result is {y} skipped, backend result is {z} skipped".format(
                    x=c_id,
                    w=be_control[statics.BE_NAME_FILED],
                    y=cli_controls[c_id][_CLI_RESOURCE_COUNTERS_FIELD][
            ...
            result_count = BaseKubescape.test_status_text(
                ...
                skipped_controls=be_control.get(
                    statics.BE_SKIPPED_RESOURCES_COUNT_FIELD, 0
                ),
    ...
    def test_number_of_resources_in_be(cli_result: dict, be_resources: list):
        ...
        assert len(be_resources) == cli_result[_CLI_FAILED_RESOURCES_FIELD], (
            message.format(
                x=cli_result[_CLI_FAILED_RESOURCES_FIELD], y=len(be_resources)
            )
        )

    @staticmethod
    def get_attributes_from_be_resources(
        be_resources: list, cluster: str, kind: str, name: str, namespace: str
    ):
        for resource in be_resources:
            attributes = resource["designators"]["attributes"]
            if (
                attributes["cluster"] == cluster
                and attributes["kind"] == kind
                and name in attributes["name"]
                and (len(namespace) == 0 or attributes["namespace"] == namespace)
            ):
                return [
                    control
                    for x in resource["statusToControls"].values()
                    for control in x
                ]
        return None

    @staticmethod
    def resource_in_filed_control(
        rule_responses: list, kind: str, name: str, namespace: str
    ):
        for response in rule_responses:
            k8s_obj = response["alertObject"]["k8sApiObjects"][0]
            if (
                k8s_obj["kind"] == kind
                and name in k8s_obj["metadata"]["name"]
                and (
    ...
    def test_failed_controls_in_resource(
        self,
        cli_result: dict,
        be_resources: list,
        cluster: str,
        ...
        be_affected_controls = BaseKubescape.get_attributes_from_be_resources(
            be_resources=be_resources,
            kind=kind,
            cluster=cluster,
            name=name,
            namespace=namespace,
        ...
        if not be_affected_controls:
            raise Exception(
                "no failed controls found for- cluster: {cluster}, kind: {kind}, name: {name}, namespace: {namespace}".format(
                    cluster=cluster, kind=kind, name=name, namespace=namespace
                )
            )

        resource_id, cli_affected_controls = self.get_affected_controls_from_cli_result(
        ...
        assert sorted(be_affected_controls) == sorted(cli_affected_controls), (
            "For resource: {resource_id}, the affected controls from cli is {x}, the affected controls from be is {y}".format(
                resource_id=resource_id,
                x=sorted(cli_affected_controls),
                y=sorted(be_affected_controls),
            )
        )

    @staticmethod
    def get_resource_id_from_cli_result(
        cli_result: dict, kind: str, name: str, namespace: str, api_version: str
    ):
        resource_id = ""
        for resource in cli_result[_CLI_RESOURCES_FIELD]:
            # if resource['resourceID'].endswith("/" + namespace + "/" + kind + "/" + name):
            if (
                KubectlWrapper.get_kind_from_wl(resource["object"]) == kind
                and KubectlWrapper.get_name_from_wl(resource["object"]) == name
                and KubectlWrapper.get_namespace_from_wl(resource["object"])
                == namespace
                and KubectlWrapper.get_api_version_from_wl(resource["object"])
                == api_version
            ):
    ...
    # @staticmethod
    def get_failed_controls_from_cli_result(
        self, cli_result: dict, kind: str, name: str, namespace: str, api_version: str
    ):
        # not support rbac
        if kind == "Pod":
            name = self.kubernetes_obj.get_pod_full_name(
                namespace=namespace, partial_name=name
            )
        resource_id = BaseKubescape.get_resource_id_from_cli_result(
            cli_result=cli_result,
            kind=kind,
            name=name,
            namespace=namespace,
        ...
        assert resource_id, (
            "No resource was found whose kind={kind}, name={name}, namespace={namespace} in in cli_result->resources".format(
                kind=kind, name=name, namespace=namespace
            )
        )

        resource_result = BaseKubescape.get_resource_result_from_cli_result(
            cli_result=cli_result, resource_id=resource_id
        )
        assert resource_result, (
            "No result found whose resourceID={resource_id} in cli_result->results".format(
                resource_id=resource_id
            )
        )
        ...
        return resource_id, [
            ...
            if BaseKubescape.cli_resource_failed_in_control(
                resource_result=resource_result, c_id=control["controlID"]
            )
        ]

    def get_affected_controls_from_cli_result(
        self, cli_result: dict, kind: str, name: str, namespace: str, api_version: str
    ):
        # not support rbac
        if kind == "Pod":
        ...
        return resource_id, list(
            map(
                lambda control: control["controlID"],
                resource_result[statics.CONTROLS_FIELD],
            )
        )
    ...
    def get_top_failed_controls_from_cli(self, cli_result, top_x=5):
        ...
        sorted_controls = sorted(
            failed_controls.items(),
            key=lambda x: (-x[1]["ResourceCounters"]["failedResources"], x[0]),
        )
    ...
    def test_top_controls_from_backend(
        self, cli_result: dict, be_results: list, report_guid: str, framework_name: str
    ):
        be_ctrl_ids = [be_ctrl["id"] for be_ctrl in be_results]
        assert 1 <= len(be_ctrl_ids) <= 5, (
            f"Top controls count should be between 1 to 5 but was {len(be_ctrl_ids)}"
        )

        cl_top_failed = self.get_top_failed_controls_from_cli(
            cli_result["summaryDetails"]["controls"]
        )
        ...
        for be_ctrl in be_results:
            for id, cli_control in cl_top_failed.items():
                ...
                if be_ctrl["id"] == id:
                    if (
                        be_ctrl["clusters"][0]["resourcesCount"]
                        < cli_control["ResourceCounters"]["failedResources"]
                    ):
                        assert False, (
                            f"Control {id} Resource counters don't match. BE: {be_ctrl['clusters'][0]['resourcesCount']}, Results: {cli_control['ResourceCounters']['failedResources']} but was {be_ctrl['clusters'][0]['resourcesCount']}"
                        )
                    assert be_ctrl["clusters"][0]["reportGUID"] == report_guid, (
                        f"Control {be_ctrl['id']} reportGUID should be {report_guid} but was {be_ctrl['clusters'][0]['reportGUID']}"
                    )
                    assert (
                        be_ctrl["clusters"][0]["topFailedFramework"] == framework_name
                    ), (
                        f"Control {be_ctrl['id']} framework name should be {framework_name} but was {be_ctrl['clusters'][0]['topFailedFramework']}"
                    )
                    assert (
                        be_ctrl["clusters"][0]["resourcesCount"]
                        == cli_control["ResourceCounters"]["failedResources"]
                    ), (
                        f"Control {be_ctrl['id']} should have {cli_control['ResourceCounters']['failedResources']} failed resources but was {be_ctrl['clusters'][0]['resourcesCount']}"
                    )
                    assert be_ctrl["name"] == cli_control["name"], (
                        f"Control {be_ctrl['id']} should have name {cli_control['name']} but was {be_ctrl['name']}"
                    )
                    assert be_ctrl["baseScore"] == cli_control["scoreFactor"], (
                        f"Control {be_ctrl['id']} should have scored {cli_control['scoreFactor']} but was {be_ctrl['baseScore']}"
                    )
    ...
    def test_resources_from_backend(self, cli_result: dict, be_resources: list):
        ...
        for resource_obj in resources_obj:
            self.test_failed_controls_in_resource(
                cli_result=cli_result,
                be_resources=be_resources,
                kind=resource_obj["kind"],
                api_version=resource_obj["apiVersion"],
    ...
    def get_resource_from_be_resources(be_resources: list, resource_name: str):
        for resource in be_resources:
            if (
                resource["name"][len(resource["name"]) - len(resource_name) :]
                == resource_name
            ):
                return resource
        raise Exception(
    ...
    def test_related_applied_in_be(
        self,
        control_name: str,
        control_id: str,
        report_guid: str,
        ...
        be_resources = self.get_posture_resources(
            framework_name=framework_name,
            report_guid=report_guid,
            resource_name=resource_name,
            related_exceptions="true",
            namespace=namespace,
        )

        resource = BaseKubescape.get_resource_from_be_resources(
            be_resources=be_resources, resource_name=resource_name
        )
        assert (
            has_applied
            and len(resource["exceptionApplied"]) > 0
            or not has_applied
            and len(resource["exceptionApplied"]) == 0
        ), (
            "Applied-exception was received, "
            "even though it was not supposed to be received"
        )
        assert (
            has_related
            and len(resource["relatedExceptions"]) > 0
            or not has_related
            and len(resource["relatedExceptions"]) == 0
        ), (
            "relatedExceptions was received, "
            "even though it was not supposed to be received"
        )
        assert (
            has_related
            and len(resource["ignoreRulesSummary"]) > 0
            or not has_related
            and resource["ignoreRulesSummary"] == None
        ), (
            "ignoreRulesSummary was received, "
            "even though it was not supposed to be received"
        )

    def test_data_in_be(
        self, cli_result, cluster_name: str, framework_name: str, old_report_guid: str
    ):
        ...
        if framework_name not in statics.SECURITY_FRAMEWORKS:
            self.compare_top_controls_data(
                cli_result=cli_result,
                cluster_name=cluster_name,
                report_guid=report_guid,
                framework_name=framework_name,
    ...
    def get_posture_control_by_id(
        self, framework_name: str, report_guid: str, control_id: str
    ):
        be_controls = self.get_posture_controls(
    ...
    def test_exception_in_controls(
        self, framework_name: str, report_guid: str, control_id: str
    ):
        control = self.get_posture_control_by_id(
            framework_name=framework_name,
            report_guid=report_guid,
            control_id=control_id,
        )
        message = (
            "Expect to reduce the failed-controls. received from backend: previousFailedResourcesCount = {x1}"
            " and failedResourcesCount = {x2}"
        )
        assert (
            control["previousFailedResourcesCount"] - control["failedResourcesCount"]
            > 0
        ), message.format(
            x1=control["previousFailedResourcesCount"],
            x2=control["failedResourcesCount"],
        )

    def get_report_guid_and_repo_hash_for_git_repository(
        self,
        ...
        for _ in range(check_intervals):
            try:
                repository_scans = self.get_repository_posture_repositories(
                ...
                Logger.logger.exception(
                    "get_repository_posture_repositories returned with error"
                )
                repository_scans = []
            ...
            scan = next(
                (
                    scan
                    for scan in repository_scans
                    if scan[statics.BE_REPORT_GUID_FIELD] != old_report_guid
                ),
                None,
            )
            if scan:
                return scan[statics.BE_REPORT_GUID_FIELD], scan["designators"][
                    "attributes"
                ]["repoHash"]
    ...
    def compare_framework_data(self, cli_result, framework_name, report_guid):
        be_frameworks = self.get_posture_frameworks(
            framework_name=framework_name, report_guid=report_guid
        )
        self.test_frameworks_from_backend(
            be_frameworks=be_frameworks, cli_result=cli_result
        )
    ...
    def compare_controls_data(self, cli_result, framework_name, report_guid):
        ...
        self.test_controls_from_backend(
            be_controls=be_controls,
            be_frameworks=be_frameworks,
            cli_controls=cli_controls,
        )
    ...
    def compare_resources_data(self, cli_result, framework_name, report_guid):
        ...
        self.test_resources_from_backend(
            be_resources=be_resources, cli_result=cli_result
        )

    def compare_top_controls_data(
        self, cli_result, cluster_name, report_guid, framework_name
    ):
        be_results = self.get_top_controls_results(cluster_name)
        self.test_top_controls_from_backend(
            cli_result=cli_result,
    ...
    def post_posture_exception(self, exceptions_file, cluster_name: str):
        ks_exceptions = self.create_ks_exceptions(
            cluster_name=cluster_name, exceptions_file=exceptions_file
        )
        if isinstance(ks_exceptions, list):
            return [
                self.post_posture_exception(
                    exceptions_file=i, cluster_name=cluster_name
                )
                for i in exceptions_file
            ]
    ...
    def delete_posture_exception(self, policy_guid):
        ...
        self.wait_for_report(
            report_type=self.backend.delete_posture_exception,
            policy_guid=policy_guid["guid"],
        )

    def post_custom_framework(self, framework_file, cluster_name: str):
        framework_name, ks_custom_fw = self.create_ks_custom_fw(
            cluster_name=cluster_name, framework_file=framework_file
        )
        if isinstance(ks_custom_fw, list):
            return [
                self.post_custom_framework(framework_file=i, cluster_name=cluster_name)
                for i in ks_custom_fw
            ]
        elif isinstance(ks_custom_fw, dict):
            report_fw, _ = self.wait_for_report(
                report_type=self.backend.post_custom_framework, fw_object=ks_custom_fw
            )
    ...
    def delete_custom_framework(self, ks_custom_fw):
        ...
        self.wait_for_report(
            report_type=self.backend.delete_custom_framework,
            framework_name=ks_custom_fw["name"],
        )

    def delete_kubescape_config_map(
        self, namespace: str = "default", name: str = "kubescape"
    ):
        try:
    ...
    @staticmethod
    def test_controls_count_of_custom_fw(
        fw_object: dict, ks_custom_fw: dict, be_result: dict
    ):
        message = "Create custom-fw with {x} controls, but backend in framework return {y} controls"
        assert len(fw_object["controls"]) == len(ks_custom_fw["controlsIDs"]), (
            message.format(
    ...
    def test_result_of_custom_fw(cli_controls: dict, be_result: list):
        ...
        assert len(cli_controls) == len(be_controls), message.format(
            x=len(cli_controls), y=len(be_controls)
        )
        for be_control in be_controls:
            c_id = be_control["id"]
            message = (
                "In control {i} cli-result return {x} failedResources and be-result return {y}"
            ...
            assert (
                ...
                i=c_id,
                x=cli_controls[c_id][_CLI_RESOURCE_COUNTERS_FIELD][
                    _CLI_FAILED_RESOURCES_FIELD
                ],
                y=be_control[statics.BE_FAILED_RESOURCES_COUNT_FIELD],
            )

    def test_scan_custom_fw_result(
        self, cli_result: dict, fw_object: dict, report_guid: str
    ):
        ks_custom_fw = self.get_framework(framework_name=fw_object["name"])
        be_fw_result = self.get_posture_frameworks(
            framework_name=fw_object["name"], report_guid=report_guid
        )[0]
        self.test_controls_count_of_custom_fw(
            fw_object=fw_object, ks_custom_fw=ks_custom_fw, be_result=be_fw_result
        )
        be_controls_result = self.get_posture_controls(
            framework_name=fw_object["name"], report_guid=report_guid
        )
        self.test_result_of_custom_fw(
            cli_controls=cli_result[_CLI_SUMMARY_DETAILS_FIELD][statics.CONTROLS_FIELD],
    ...
    def test_scan_custom_fw_deleted(self, fw_name: str):
        ...
        assert fw_name not in frameworks_name, (
            "Backend return framework {x} after deleted".format(x=fw_name)
        )

    @staticmethod
    def test_customer_configuration_result(
        cli_result: dict, expected_result: str, c_id: str
    ):
        assert (
            c_id
            in cli_result[_CLI_SUMMARY_DETAILS_FIELD][statics.CONTROLS_FIELD].keys()
        ), "Not found control {c_id} in cli_results->summaryDetails".format(c_id=c_id)
        assert (
            cli_result[_CLI_SUMMARY_DETAILS_FIELD][statics.CONTROLS_FIELD][c_id][
                _CLI_STATUS_FILED
            ]
            == expected_result
        ), (
            "Received from cli_results for control {c_id} status {x}, but expected to be {y}".format(
                c_id=c_id,
                x=cli_result[_CLI_SUMMARY_DETAILS_FIELD][statics.CONTROLS_FIELD][c_id][
                    _CLI_STATUS_FILED
                ],
                y=expected_result,
            )
    ...
    def test_control_input_in_kubescape(
        self, input_kind: str, input_name: str, included: bool
    ):
    ...
    def add_to_customer_configuration(
        self, customer_configuration: dict, input_kind: str, input_name: str
    ):
        customer_configuration[_SETTINGS_FILED][_POSTURE_CONTROL_INPUT_FILED][
            input_kind
        ].append(input_name)
        self.update_customer_configuration(customer_config=customer_configuration)

    def remove_from_customer_configuration(
        self, customer_configuration: dict, input_kind: str, input_name: str
    ):
        if (
            input_name
            in customer_configuration[_SETTINGS_FILED][_POSTURE_CONTROL_INPUT_FILED][
                input_kind
            ]
        ):
            customer_configuration[_SETTINGS_FILED][_POSTURE_CONTROL_INPUT_FILED][
    ...
    def test_expected_result_against_cli_result(
        self, cli_result: dict, expected_result_name: str
    ):
        expected_r = self.create_kubescape_expected_results(
            expected_results=expected_result_name
        )
        cli_r = cli_result[_CLI_SUMMARY_DETAILS_FIELD]
        fail_path = self.expected_results_contained_in_cli_results(
            er=expected_r, cr=cli_r, failed_path=""
        )
        assert not fail_path, (
            "Expected that all the fields in expected-results will be in cli-results. "
        ...
        for k in er.keys():
            if isinstance(er[k], str):
                ...
                if k not in cr.keys():
                    ...
                    result.extend(
                        BaseKubescape.expected_results_contained_in_cli_results(
                            er=er[k], cr=cr[k], failed_path=failed_path + k
                        )
                    )
            else:
                raise Exception(
                    f"value in Expected-result could be only str, int, float, dict. received {type(er[k])}"
                )
        return result
    ...
    def comapare_backend_vs_kubescape_frameworks(self, kbs_frameworks_r, be_frameworks):
        ...
        for bf in be_frameworks:
            for kf in kbs_frameworks_r:
                if bf["name"] == kf["name"]:
                    total_kube_c, failed_kube_c = self.get_total_and_failed_controls(
                        kf["controls"]
                    )
        ...
        for i in range(1, 15):
            try:
                ...
                if res.status_code >= 200 and res.status_code < 300:
                    ...
                    if "type" not in res:
                        Logger.logger.warning(
                            "response from kubescape is missing 'type'"
                        )
            ...
            time.sleep(20)
        raise Exception(
            "the last scan results are not ready in {} seconds".format(10 * 20)
        )
    ...
    def get_kubescape_as_server_last_result(self, cluster_name, port, report_guid=""):
        if self.results_ready(cluster_name, port=port, report_guid=report_guid):
            ...
            raise Exception(
                "failed to get the last kubescape scan results http code {}".format(
                    res.status_code
                )
            )
    ...
    def test_backend_vs_kubescape_result(self, report_guid, kubescape_result):
        ...
        if self.enable_security:
            for sf in statics.SECURITY_FRAMEWORKS:
                be_current_security_framework = self.get_posture_frameworks(
                    report_guid=report_guid, framework_name=sf
                )
                if be_current_security_framework:
                    Logger.logger.debug(
                        f"test_backend_vs_kubescape_result - found security framework: {sf} in backend"
                    )
                    be_frameworks.extend(be_current_security_framework)

        assert _CLI_SUMMARY_DETAILS_FIELD in kubescape_result, (
            "expected key {} is not in kubescape result,kubescape_result: {}".format(
                _CLI_SUMMARY_DETAILS_FIELD, kubescape_result
            )
        )
        kbs_r = kubescape_result[_CLI_SUMMARY_DETAILS_FIELD]

        assert _CLI_FRAMEWORKS_FIELD in kbs_r, (
            "expected key {} is not in kubescape result,kubescape_result[{}]: {}".format(
        ...
        fail_backend_frameworks, fail_kubescape_frameworks = (
            self.comapare_backend_vs_kubescape_frameworks(
                kbs_frameworks_r, be_frameworks
            )
        )
        assert not fail_backend_frameworks and not fail_kubescape_frameworks, (
            "Expected that all the fields in expected-results will be in cli-results. "
    ...
    def get_report_guid(
        ...
        found = False
        Logger.logger.info(
            "cluster_name: {}, framework_name: {}, old_report_guid: {}, wait_to_result: {}".format(
        ...
        for i in range(check_intervals):
            ...
            if len(be_cluster_overtime) > 0 and (
                old_report_guid == ""
                or be_cluster_overtime[statics.BE_CORDS_FIELD][
                    statics.BE_REPORT_GUID_FIELD
                ]
                != old_report_guid
            ):
                if found:
                    Logger.logger.info(
                        "get_report_guid - returning found report_guid: {}".format(
                            be_cluster_overtime[statics.BE_CORDS_FIELD][
                                statics.BE_REPORT_GUID_FIELD
                            ]
                        )
                    )
                    return be_cluster_overtime[statics.BE_CORDS_FIELD][
                        statics.BE_REPORT_GUID_FIELD
                    ]
                Logger.logger.info(
                    "get_report_guid - found report_guid: {}".format(
                        be_cluster_overtime[statics.BE_CORDS_FIELD][
                            statics.BE_REPORT_GUID_FIELD
                        ]
                    )
    ...
    def test_controls_compliance_score(self, report: dict):
        ...
        for _, control in report[_CLI_SUMMARY_DETAILS_FIELD][_CONTROLS_FIELD].items():
            ...
            for resource_group, amount in control[_CLI_RESOURCE_COUNTERS_FIELD].items():
                ...
                if (
                    resource_group == _CLI_PASSED_RESOURCES_FIELD
                    or resource_group == _CLI_EXCLUDED_RESOURCES_FIELD
                ):
                    passed_resources += amount
            if total_resources == 0:
                if (
                    control[_CLI_STATUS_INFO_FIELD][_CLI_STATUS_FILED]
                    == _CLI_STATUS_PASSED
                ):
                    assert control[_COMPLIANCE_SCORE_FIELD] == 100, (
                        "expected passed control to have compliance score of 100, but it is {}".format(
                            control[_COMPLIANCE_SCORE_FIELD]
                        )
                    )
                else:
                    assert control[_COMPLIANCE_SCORE_FIELD] == 0, (
                        "expected compliance score to be 0, but it is {}".format(
                            control[_COMPLIANCE_SCORE_FIELD]
                        )
                    )
            else:
                assert round(control[_COMPLIANCE_SCORE_FIELD], 2) == round(
                    (passed_resources / total_resources) * 100, 2
                ), "expected compliance score to be {}, but it is {}".format(
                    round((passed_resources / total_resources) * 100, 2),
                    round(control[_COMPLIANCE_SCORE_FIELD]),
                    2,
                )
    ...
    def test_frameworks_compliance_score(self, report: dict):
        ...
        for framework in report[_CLI_SUMMARY_DETAILS_FIELD][_CLI_FRAMEWORKS_FIELD]:
            ...
            assert round(framework[_COMPLIANCE_SCORE_FIELD], 2) == round(
                sum_scores / len(framework[_CONTROLS_FIELD]), 2
            ), "expected compliance score to be {}, but it is {}".format(
                round(sum_scores / len(framework[_CONTROLS_FIELD]), 2),
                round(framework[_COMPLIANCE_SCORE_FIELD]),
                2,
            )

    def test_controls_compliance_score_from_backend(
        self, framework_report, report_guid, framework_name: str = ""
    ):
        controls = self.get_posture_controls(
            framework_name=framework_name, report_guid=report_guid
        )
        for control in controls:
            ...
            assert (
                control[_COMPLIANCE_SCORE_FIELD]
                == framework_report[_CONTROLS_FIELD][c_id][_COMPLIANCE_SCORE_FIELD]
            ), (
    ...
    def test_frameworks_compliance_score_from_backend(
        self, framework_report, report_guid, framework_name: str = ""
    ):
        be_frameworks = self.get_posture_frameworks(
            framework_name=framework_name, report_guid=report_guid
        ...
        assert (
            framework_report[_COMPLIANCE_SCORE_FIELD]
            == be_frameworks[0][_COMPLIANCE_SCORE_FIELDV1]
    ...
    def test_compliance_score_in_clusters_overtime(
        self, cluster_name: str, framework_report: dict, framework_name: str = ""
    ):
        be_cluster_overtime = self.get_posture_clusters_overtime(
            cluster_name=cluster_name, framework_name=framework_name
        )
        assert len(be_cluster_overtime) > 0, (
            "expected to get response from clustersOvertime for framework {} in cluster {}".format(
        ...
        assert (
            be_cluster_overtime[statics.BE_CORDS_FIELD][_COMPLIANCE_SCORE_FIELD]
            == framework_report[_COMPLIANCE_SCORE_FIELD]
        ), "expected framework: {} compliance score in be to be {}, but it is".format(
            framework_name,
            framework_report[_COMPLIANCE_SCORE_FIELD],
    ...
    def test_host_scanner_results(cli_results: dict):
        for control_id, control_value in cli_results[_CLI_SUMMARY_DETAILS_FIELD][
            statics.CONTROLS_FIELD
        ].items():
    ...
    def parse_cli_args(self, **kwargs):
        ...
        assert "trigger" in kwargs["args"], (
            'cli args must contain "trigger", to understand which operation do you want to trigger.("configurations" or "vulnerabilities")'
        )
        ...
        if trigger == "configurations":
            ...
            if "include-namespaces" in kwargs["args"]:
                cli_args.extend(
                    [
                        "--include-namespaces",
                        BaseKubescape.parse_cli_args_in_list_to_string(
            ...
            raise Exception(
                'cli args must contain neither "configurations" or "vulnerabilities"'
            )
    ...
    def get_trigger_cli_full_command(self, cli_args: list):
        ...
        assert len(command) >= 4, (
            "minimum cli args should be 3 but we got less. got command: {command}".format(
                command=command
            )
        )
        return command
    ...
    def trigger_in_cluster_components(self, cli_args: str, **kwargs):
        ...
        Logger.logger.info(
            "trigger_in_cluster_components. command: {command}".format(command=command)
        )
        status_code, res = TestUtil.run_command(
            command_args=command,
            timeout=360,
    ...
    def validate_cluster_trigger_as_expected(self, cluster_name: str, **kwargs):
        assert "trigger" in kwargs["args"], (
            'cli args must contain "trigger", to understand which operation do you want to trigger.("config" or "vulnerabilities")'
        )
        trigger = kwargs["args"]["trigger"]

        if trigger == "configurations":
            self.validate_scan_trigger(
                cluster_name=cluster_name, deployment_name="kubescape", kwargs=kwargs
            )
        elif trigger == "vulnerabilities":
            self.validate_scan_trigger(
                cluster_name=cluster_name, deployment_name="operator", kwargs=kwargs
            )
        else:
            raise Exception(
                'cli args must contain neither "configurations" or "vulnerabilities"'
    ...
    @staticmethod
    def parse_string_to_found_in_list_argument(
        prefix: str, argument: list, postfix: str
    ):
        ...
        if trigger == "configurations":
            if "include-namespaces" in kwargs["args"]:
                string_to_be_found.append(
                    BaseKubescape.parse_string_to_found_in_list_argument(
                        '\\"includeNamespaces\\":[',
            ...
            if "include-namespaces" in kwargs["args"]:
                for ns in kwargs["args"]["include-namespaces"]:
                    string_to_be_found.append(
                        '{\\"commandName\\":\\"scan\\",\\"wildWlid\\":\\"wlid://cluster-'
                        + cluster_name
                        + "/namespace-"
                        + ns
                    )
    ...
    def validate_scan_trigger(self, cluster_name: str, deployment_name: str, kwargs):
        ...
        pods = self.get_pods(
            namespace="kubescape", wlid=wlid, include_terminating=False
        )
        for pod in pods:
        ...
        string_to_be_found = BaseKubescape.get_string_to_be_found(
            cluster_name=cluster_name, kwargs=kwargs
        )
        ...
        if deployment_name in desired_pod.metadata.name:
            ...
            for string in string_to_be_found:
                try:
                    self.find_string_in_log(
                        namespace="kubescape",
                        pod_name=desired_pod.metadata.name,
                        containers=deployment_name,
                        previous=False,
                        string_in_log=string,
                    )
        ...
        assert string_to_be_found_counter == len(string_to_be_found), (
            "not all expected string found in the log. pod log: {pod_log}".format(
                pod_log=self.get_pod_logs(
                    namespace="kubescape",
                    pod_name=pod_name,
                    containers=deployment_name,
                    previous=False,
