...
from tests_scripts.workflows.utils import get_messages_from_slack_channel, enrich_slack_alert_channel, get_messages_from_teams_channel, enrich_teams_alert_channel, get_tickets_from_jira_channel, get_tickets_from_linear_channel
...
class WorkflowsTests(object):
    ...
    @staticmethod
    def linear_notifications_workflows():
        from tests_scripts.workflows.linear_workflows import WorkflowsLinearNotifications
        return TestConfiguration(
            name=inspect.currentframe().f_code.co_name,
            test_obj=WorkflowsLinearNotifications,
            deployments=join(DEFAULT_WORKFLOWS_DEPLOYMENT_PATH, "http1"),
            getMessagesFunc=get_tickets_from_linear_channel,
            create_test_tenant = False
        )    
