...
def parallelStagesMap = tests.collectEntries {
    ["${it.key}" : generate_stage(it.value[1], it.key, it.value[0], "${backend}", (tests_to_skip.size() != 0 && tests_to_skip.containsKey("${backend}") && tests_to_skip["${backend}"].contains("${it.key}")))]
}
